    ...
    a match {
      ...
      case AExp.Deref(d) => {
        if sigma.contains(d) then {
          assert(
            heap.contains(sigma(d)),
            s"expect address ${sigma(d)} to be valid in heap"
          )
          heap(sigma(d))
        } else {
    ...
    i match {
        ...
        val x = a.x match
          ...
          case _ =>
            throw InvalidParameter(
              "expect `a.x` to be a variable or reference/pointer"
            )
      ...
      case ImpStmt.Alloc(a) => {
        val ref = a.ref match {
          ...
          case _ =>
            throw InvalidParameter(
              s"expect ref/pointer type for allocation, but get ${a.ref}"
            )
        }
      ...
      case ImpStmt.Store(s) => {
        val deref = s.deref match {
          ...
          case _ =>
            throw InvalidParameter(
              s"expect deref for store, but get ${s.deref}"
            )
        }
